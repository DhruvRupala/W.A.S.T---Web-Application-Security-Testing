...
const Scan = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <div className="mb-12">
        <h2 className="text-4xl font-black uppercase tracking-widest mb-2 flex items-center gap-4">
          <Target className="text-cyber-pink" size={40} />
          New Target Scan
        </h2>
        <p className="text-cyber-blue text-sm max-w-2xl" style={{ fontFamily: "'Fira Code', monospace" }}>
          Initialize a comprehensive security vulnerability scan against the specified target.
        </p>
      </div>

      <div className="glass-panel p-8 border border-cyber-pink/30 relative overflow-hidden">
        {scanning && (
        ...
        <form onSubmit={handleScan} className="space-y-8 relative z-10">
          <div>
            <label className="block text-sm text-gray-400 mb-3 uppercase tracking-widest" style={{ fontFamily: "'Fira Code', monospace" }}>Target Endpoint URL</label>
            <div className="flex gap-4">
              <input
                type="text"
                className="cyber-input text-lg py-4 flex-1 border-gray-700 bg-cyber-black focus:border-cyber-pink focus:ring-cyber-pink"
              ...
              <button
                type="submit"
                disabled={scanning}
                className="relative inline-flex items-center justify-center px-8 py-4 overflow-hidden font-bold tracking-widest text-white uppercase bg-cyber-pink/20 border border-cyber-pink hover:bg-cyber-pink/40 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed w-48"
              >
                {scanning ? (
            ...
            {error && <p className="text-cyber-pink text-sm mt-3 flex items-center gap-2" style={{ fontFamily: "'Fira Code', monospace" }}><Shield size={14} /> {error}</p>}
          </div>
        </form>
        ...
        {scanning && (
          <div className="mt-8 pt-6 border-t border-gray-800">
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm text-cyber-blue uppercase tracking-widest" style={{ fontFamily: "'Fira Code', monospace" }}>Scan Progress</span>
              <span className="text-sm text-cyber-pink font-bold" style={{ fontFamily: "'Fira Code', monospace" }}>{progress}%</span>
            </div>
            <div className="w-full h-3 bg-cyber-black border border-gray-800 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-700 ease-out"
                style={{ width: `${progress}%`, background: 'linear-gradient(90deg, #00f0ff, #ff003c)', boxShadow: '0 0 12px rgba(0,240,255,0.5)' }}
              ></div>
            </div>
            <p className="text-xs text-gray-400 mt-2 animate-pulse" style={{ fontFamily: "'Fira Code', monospace" }}>{statusMsg}</p>
          </div>
        )}
        ...
        {scanResult && !scanning && (
          <div className="mt-8 pt-6 border-t border-gray-800">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-lg font-bold uppercase">{scanResult.status === 'completed' ? 'Scan Complete' : 'Scan Failed'}</span>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-gray-800">
          <h4 className="text-cyber-blue uppercase text-sm mb-6 pb-2 border-b border-gray-800 inline-block" style={{ fontFamily: "'Fira Code', monospace" }}>Active Modules</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {['XSS Injector', 'SQLi Scanner', 'Port Discovery', 'Security Headers'].map(mod => (
              <div key={mod} className="flex items-center gap-3 p-3 bg-cyber-black border border-gray-800 rounded-sm">
                ...
                <span className="text-sm text-gray-300" style={{ fontFamily: "'Fira Code', monospace" }}>{mod}</span>
